...
function indexOfNull<T extends DataType>(data: Data<T>, fromIndex?: number): number {
    ...
    let i = 0;
    for (const isValid of iterateBits(nullBitmap, data.offset + (fromIndex || 0), data.length, nullBitmap, getBool)) {
        if (!isValid) { return i; }
        ++i;
    }
    return -1;
...
function indexOfValue<T extends DataType>(data: Data<T>, searchElement?: T['TValue'] | null, fromIndex?: number): number {
    ...
    const vector = new Vector(data.type, [data], new Uint32Array([0, data.length]));
