...
/** @ignore */
const getBigInts         = <T extends Numeric2X>({ stride, values, type }: Data<T>, index: number): T['TValue'] => <any> BN.new(values.subarray(stride * index, stride * (index + 1)), type.isSigned);
/** @ignore */
...
/** @ignore */
const getInt = <T extends Int>(data: Data<T>, index: number): T['TValue'] => (
    data.type.bitWidth < 64
        ? getNumeric(data as Data<Numeric1X>, index)
        : getBigInts(data as Data<Numeric2X>, index)
);

/* istanbul ignore next */
/** @ignore */
const getFloat = <T extends Float> (data: Data<T>, index: number): T['TValue'] => (
    data.type.precision !== Precision.HALF
        ? getNumeric(data as Data<Numeric1X>, index)
        : getFloat16(data as Data<Float16>, index)
);
...
/** @ignore */
const getTimeSecond      = <T extends TimeSecond>     ({ values, stride }: Data<T>, index: number): T['TValue'] => values[stride * index];
/** @ignore */
const getTimeMillisecond = <T extends TimeMillisecond>({ values, stride }: Data<T>, index: number): T['TValue'] => values[stride * index];
/** @ignore */
const getTimeMicrosecond = <T extends TimeMicrosecond>({ values         }: Data<T>, index: number): T['TValue'] => BN.signed(values.subarray(2 * index, 2 * (index + 1)));
/** @ignore */
const getTimeNanosecond  = <T extends TimeNanosecond> ({ values         }: Data<T>, index: number): T['TValue'] => BN.signed(values.subarray(2 * index, 2 * (index + 1)));
/* istanbul ignore next */
/** @ignore */
...
const getList = <T extends List>(data: Data<T>, index: number): T['TValue'] => {
    ...
    const child: Data<T['valueType']> = data.childData[0];
