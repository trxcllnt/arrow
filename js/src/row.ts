...
abstract class Row<TKey extends PropertyKey = any, TVal = any> implements Map<TKey, TVal> {
    ...
    protected [kKeyToIdx]: Map<TKey, number>;
    protected [kIdxToVal]: TVal[];
    ...
    public abstract keys(): IterableIterator<TKey>;
    public abstract values(): IterableIterator<TVal>;
    public abstract getKey(idx: number): TKey;
    public abstract getIndex(key: TKey): number;
    public abstract getValue(idx: number): TVal;
    public abstract setValue(idx: number, val: TVal): void;

    public entries() { return this[Symbol.iterator](); }

    public has(key: TKey) { return this.get(key) !== undefined; }
    ...
    public delete(_: TKey): boolean { throw new Error(`Deleting ${this[Symbol.toStringTag]} values not supported.`); }
    ...
    public *[Symbol.iterator](): IterableIterator<[TKey, TVal]> {
        ...
        for (let k: TKey, v: TVal, i = 0, kr: IteratorResult<TKey>, vr: IteratorResult<TVal>;
    ...
    public forEach(callbackfn: (value: TVal, key: TKey, map: Map<TKey, TVal>) => void, thisArg?: any): void {
        ...
        const callback = thisArg === undefined ? callbackfn :
            (v: TVal, k: TKey, m: Map<TKey, TVal>) => callbackfn.call(thisArg, v, k, m);
        const ktoi = this[kKeyToIdx] || (this[kKeyToIdx] = new Map());
        const itov = this[kIdxToVal] || (this[kIdxToVal] = new Array(this.size));
...
export class MapRow<K extends DataType = any, V extends DataType = any> extends Row<K['TValue'], V['TValue'] | null> {
    constructor(data: Data<Struct<{ key: K, value: V }>>) {
        const parent = new Vector(data.type, [data], new Uint32Array([0, data.length]));
...
export class StructRow<T extends { [key: string]: DataType } = any> extends Row<keyof T, T[keyof T]['TValue'] | null> {
    constructor(data: Data<Struct<T>>) {
        const parent = new Vector(data.type, [data], new Uint32Array([0, data.length]));
