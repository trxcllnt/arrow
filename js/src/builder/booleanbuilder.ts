...
export class DictionaryEncodeBooleanBuilder<T extends boolean, TKey extends Int>
{
    ...
    public encode(source: Iterable<T>) : { dictData : Data<Bool>, keysData : Data<TKey> } {
        const indices = [-1];
        ...
        for (let fromSource of source) {
            ...
            if (this._isNull(value, sourceIndex, indexValue)) {
                ...
                data.push(...(this._encoder(value, key, sourceIndex) as number[]));
            }
        ...
        const dictData = Data.Bool(new Bool(), 0, data.length, 0, null, data);
