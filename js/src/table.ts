...
export class Table<T extends { [key: string]: DataType } = any> extends Vector<Struct<T>> {
    constructor(batches: Data<Struct<T>>[]);
    constructor(schema: Schema<T>, batches: Data<Struct<T>>[]);
    constructor(...args: any[]) {
        let schema: Schema<T> = null!;
        if (args[0] instanceof Schema) { schema = args.shift(); }
        const chunks = selectArgs<Data<Struct<T>>>(Data, args);
        if (!schema) {
            if (!chunks[0]) {
                throw new TypeError('Table must be initialized with a Schema or at least one RecordBatch');
            }
            schema = new Schema(chunks[0].type.children);
        }
        chunks[0] || (chunks[0] = Data.new(new Struct<T>(schema.fields), 0, 0));
    ...
    public assign<R extends { [key: string]: DataType } = any>(other: Table<R>) {
        ...
        const batches = distributeVectorsIntoRecordBatches<T & R>(schema, columns).slice(1);
        return new Table<T & R>(schema, batches.map((rb) => (rb as any).data as Data<Struct<T & R>>));
    }
    ...
    public selectAt<K extends T[keyof T] = any>(...columnIndices: number[]) {
        ...
        return new Table(schema, this._chunks.map(({ length, childData }) => {
            const children = columnIndices.map((i) => childData[i]);
            const type = new Struct<{ [key: string]: K; }>(schema.fields);
            return new Data(type, 0, length, 0, undefined, children.filter(Boolean));
        }));
    }
}

export class Column<T extends DataType = any> extends Vector<T> {}
...
//     public get schema() { return this.schema; }
//     public get length() { return this._length; }
//     public get chunks() { return this._chunks; }
//     public get numCols() { return this._numChildren; }

//     public clone(chunks = this._chunks) {
//         return new Table<T>(this.schema, chunks);
//     }
...
//         const fields = (this.schema as Schema<any>).fields;
